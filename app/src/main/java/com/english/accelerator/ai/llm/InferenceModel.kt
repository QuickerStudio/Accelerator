...
/**
 * Local LLM inference model for offline use
 * Based on official MediaPipe LLM inference architecture
 */
class InferenceModel private constructor(context: Context) {
    ...
    val uiState = ChatUiState(model.thinking)

    init {
        if (!modelExists(context)) {
            throw IllegalArgumentException("Model not found at path: ${model.getPath(context)}")
        }
    ...
    private fun createEngine(context: Context) {
        ...
        try {
            ...
            AppLogger.error(TAG, "Load model error: ${e.message}", e)
    ...
    private fun createSession() {
        ...
        try {
            ...
            AppLogger.error(TAG, "LlmInferenceSession create error: ${e.message}", e)
    ...
    companion object {
        var model: Model = Model.GEMMA_3N_E2B_IT_INT4
        ...
        fun modelExists(context: Context): Boolean {
            return File(model.getPath(context)).exists()
        }
    }
}
