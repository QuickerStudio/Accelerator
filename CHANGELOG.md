...
## [v2.0.0] - 2026-03-04 至 2026-03-05
...
- **流式推理（Streaming Inference）**
  - 实现实时 AI 响应生成
  - 添加自动线程标题生成功能，带淡入淡出动画
  - 升级 Markdown 库，优化流式性能
  - 添加推理性能指标显示
  - 添加互斥锁防止并发推理崩溃
  - 为 Agent 和 LLM 推理系统添加单元测试
...
#### 架构重构
- 删除过时的 GemmaInferenceManager，替换为 InferenceEngine
- 合并 core、inference、llm 包为统一的 llm 包
- 添加 prompt 模板层用于语法和写作服务
- 添加统一推理引擎核心层
- 集成官方 MediaPipe LLM 推理实现
- 添加 Agent 系统多角色 AI 架构

#### 下载系统改进
- 修复后台下载中断问题
- 改进下载 UI：显示 3 位小数进度，降低速度更新频率
- 解耦进度/速度显示与按钮状态管理
- 添加后台下载支持和状态管理
- 升级 MediaPipe 到 0.10.32 修复 JNI 库缺失
- 修复断点续传状态管理
- 添加下载路由切换功能

#### 文档
- 为所有 AI 模块添加全面的 README 文档
- 添加 Agent 系统架构综合文档
...
#### 架构重构
- 建立单词学习系统五层架构
- 重命名 WordPoolManager 为 WordPoolIndexer 并明确职责
- 集成 WordLoader 作为算法和 UI 之间的中间件层
- 完成算法模块所有核心组件
- 创建原子化单词推送算法模块
...
- 添加学习统计屏幕，带日历和详细指标
- 添加专业时间选择器和学习计划设置
- 使用日历日期选择重构 LearningPlanScreen
- 添加时间验证和夜间勿扰功能
- 实现双备份系统，带版本控制和数据完整性检查

#### 设置界面
- 实现模块化设置界面，带全面的功能卡片
- 重构为容器-组件架构
- 添加内部文件浏览器替换崩溃的外部文件管理器
- 添加截图和数据导出/导入功能
- 添加权限管理和运行时权限请求

#### 下载系统
- 添加 Config.json 状态管理和 ConfigManager
- 添加下载状态管理系统支持断点续传
- 改进下载验证，带文件大小验证
- 添加模型文件完整性检查
- 切换到 ModelScope 下载源
...
## [v0.5.0] - 2026-03-02 至 2026-03-03

### 对话功能 + AI 模型集成

#### AI 集成
- 从 Gemma-2B 升级到 Gemma 3n E2B with LiteRT
- 添加双路由下载支持
- 添加智能模型下载，带 ping 检测和本地文件检查
- 集成 Gemma-2B 推理到 WritingScreen
- 在 MainActivity 中初始化 GemmaInferenceManager
- 添加 Gemma-2B LLM 基础设施：MediaPipe 依赖和 AI 包
- 添加应用启动时自动模型初始化

#### 对话功能
- 实现微信风格语音录制功能
- 添加语音/文本模式切换按钮
- 完善发送按钮位置（7:3 比例，发送按钮在文本框内）
- 将 AI 对话功能集成到 SpeakingScreen
- 构建 SpeakingScreen，带白色主题和消息气泡
- 添加新对话线程按钮

#### 会话管理
- 添加 HistoryManager 用于对话历史管理
- 添加 ConversationHistory 数据模型
- 添加 Session 数据模型用于会话管理系统
- 添加对话历史删除、下载暂停/恢复和路由切换
...
#### 基础设施
- 设置日志和配置管理基础设施
- 添加 ConfigManager 内联函数
- 初始化 SessionManager 和 HistoryManager
